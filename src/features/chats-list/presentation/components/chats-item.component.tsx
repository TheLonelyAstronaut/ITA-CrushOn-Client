...
export const ChatsItem: React.FC<ChatsItemProps> = (props: ChatsItemProps) => {
    const user = CardsData.find(user => { if (user.id === props.userId) return true});
    return (
        <ChatsItemView onPress={() => props.navigation.navigate('Chat',{id: props.userId})}>
            ...
            <Date>{Messages[0].createdAt.getHours().toString()}:{Messages[0].createdAt.getMinutes().toString()}</Date>
        </ChatsItemView>
    );
};